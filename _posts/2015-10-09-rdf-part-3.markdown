...
PREFIX oscys: <http://cdrhsites.unl.edu/data/projects/oscys/rdf/oscys.objectproperties.owl#>

SELECT *
FROM <http://earlywashingtondc.org/relationships.ttl>
WHERE {
  // query goes here
}
{% endhighlight %}

This should look mostly familiar, if you were following along at home with part II, except for the PREFIX declarations at the top.  The prefix is a tool to make things more human readable.  It's much nicer to get a result that says `osrdf:per.000001` rather than one that says `<http://cdrhsites.unl.edu/data/projects/oscys/rdf/oscys.relationships#per.000001>`.
...
SELECT *
FROM <http://earlywashingtondc.org/ontology.owl>
WHERE {
